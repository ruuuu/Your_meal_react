...
export const ModalDelivery = () => {
  ...
  // деструткризация:
  const { isOpen } = useSelector((state) => {   // useSelector -хук
    return state.modal;                        // state.isOpen  взяли из itinitialState ModalDeliverySlice.js
  });

  const dispatch = useDispatch();   //  чтобы получить action. Вернет фукнцию


  return (isOpen && (    // если isOpen=true, то отображаем верстку модалки

    <div className={style.modal} onClick={({ target, eventTarget }) => {  // target это то элемент на который нажали
      console.log('target ', target);
      conole.log('eventTarget ', eventTarget);
